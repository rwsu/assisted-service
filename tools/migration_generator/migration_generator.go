...
func main() {
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "Failed to create migration file %s: %s", filePath, err)
		os.Exit(1)
	}

	testFile, err := os.Create(testFilePath)
	if err != nil {
		fmt.Fprintln(os.Stderr, "Failed to create migration test file %s: %s", testFilePath, err)
		os.Exit(1)
	}

	err = template.Must(template.New("migration").Parse(migrationTemplate)).Execute(file, data)
	if err != nil {
		fmt.Fprintln(os.Stderr, "Failed to write migration file %s: %s", filePath, err)
		os.Exit(1)
	}

	err = template.Must(template.New("migrationTest").Parse(migrationTestTemplate)).Execute(testFile, data)
	if err != nil {
		fmt.Fprintln(os.Stderr, "Failed to write migration file %s: %s", testFilePath, err)
		os.Exit(1)
	}
}
...
var _ = Describe("{{.FuncName}}", func() {
	...
	BeforeEach(func() {
		db = common.PrepareTestDB("{{.FuncNameSnake}}", &events.Event{})
	})
